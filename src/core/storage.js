...
export const StorageService = {
    getWatchlist: () => safeParseJSON(getFirstStoredValue([STORAGE_KEYS.WATCHLIST, 'cinerank_watchlist', 'watchlist']), []),
    ...
    addToWatchlist: (item, currentType) => {
        ...
        const nextWatchlist = watchlist.filter(saved => saved.id !== itemToSave.id);
    ...
    getTheme: () => getFirstStoredValue([STORAGE_KEYS.THEME, 'cinerank_theme', 'theme']),
    setTheme: (theme) => {
        if (typeof localStorage === 'undefined') return;
        localStorage.setItem(STORAGE_KEYS.THEME, theme);
    },

    getHighScore: () => Number(getFirstStoredValue([STORAGE_KEYS.HIGH_SCORE, 'highScore']) || 0),
    setHighScore: (score) => {
        if (typeof localStorage === 'undefined') return;
        localStorage.setItem(STORAGE_KEYS.HIGH_SCORE, String(score));
    },

    getLastSeenNewsDate: () => getFirstStoredValue([STORAGE_KEYS.LAST_SEEN_NEWS, 'cinerank_last_news', 'lastSeenNewsDate']),
    setLastSeenNewsDate: (dateStr) => {
        if (typeof localStorage === 'undefined') return;
        localStorage.setItem(STORAGE_KEYS.LAST_SEEN_NEWS, dateStr);
    },

    getSearchHistory: () => safeParseJSON(getFirstStoredValue([STORAGE_KEYS.SEARCH_HISTORY, 'searchHistory']), []),
